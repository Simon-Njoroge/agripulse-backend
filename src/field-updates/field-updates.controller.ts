...
export class FieldUpdatesController {
  ...
  async createUpdate(
    @Body() createDto: CreateFieldUpdateDto,
    @Req() req: RequestWithUser,
  ) {
    const result = await this.fieldUpdatesService.createUpdate(
      createDto,
      (req as any).user.sub,
    );
    return result;
  ...
  async addNote(@Body() addNoteDto: AddNoteDto, @Req() req: RequestWithUser) {
    const result = await this.fieldUpdatesService.addNote(
      addNoteDto,
      (req as any).user.sub,
    );
    return result;
  ...
  async bulkUpdateStages(
    ...
    const result = await this.fieldUpdatesService.bulkUpdateStages(
      bulkUpdateDto,
      (req as any).user.sub,
    );
    return result;
  ...
  async getMyUpdates(
    ...
    const result = await this.fieldUpdatesService.getMyUpdates(
      (req as any).user.sub,
      filters,
    );
    return result;
  ...
  async deleteUpdate(@Param('id') id: string, @Req() req: RequestWithUser) {
    const result = await this.fieldUpdatesService.deleteUpdate(
      id,
      (req as any).user.sub,
    );
    return result;
